...
def test_returns_build_ids(jenkins, job):
    print(jenkins.has_job("Fahrzeug-Server"))
    buildIdIterator = query_a_build.getBuildNumbersToJob("Fahrzeug-Server", jenkins)
    buildIdList = list(buildIdIterator)
    print("buildIdList")
    print(buildIdList)
    assert len(buildIdList) == 6

def test_fahrzeugserver_unstable_duration(jenkins, job, monkeypatch, build):
    jenkins.get_job  = mock.MagicMock(return_value=job)
    job.get_build    = mock.MagicMock(return_value=build)
    timestamp = 1583495750
    result = "UNSTABLE"
    testDate = date(2020, 3, 6)
    expected = 12 + 5/60
    result = query_a_build.getUnstableHoursForJobAtDate(job, testDate)
