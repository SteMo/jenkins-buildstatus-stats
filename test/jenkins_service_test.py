...
def jenkins(monkeypatch):
    def fake_get_jenkins_poll(cls, tree=None):  # pylint: disable=unused-argument
        return JENKINS_DATA
    def fake_job_data(cls, url, tree=None):  # pylint: disable=unused-argument
        return JOB_DATA           
...
def create_build_json(job, buildnumber, monkeypatch):
  ...
  return Build('http://localhost:800/view/Buildampel/job/Fahrzeug-Server/3119/', buildnumber, job)


def test_returns_builds_of_job(jenkins, job, monkeypatch):
    build_id_list = [3123,3122,3121,3120,3119,3112]
    build_list = []
    for build_id in build_id_list:
      build_list.append(create_build_json(job, build_id, monkeypatch))
    job.get_build = mock.MagicMock(side_effect=build_list)

    job_dto = jenkins_service.get_builds_from_build_ids(job, build_id_list)
        
    assert job_dto != None
    build_list = job_dto.get_build_list()
    assert len(build_list) == 6
    assert build_list[0].get_build_id() == 3123
    assert build_list[5].get_build_id() == 3112
