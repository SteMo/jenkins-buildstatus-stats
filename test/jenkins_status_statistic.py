...
def test_lists_job_fail_times_per_day():
    url = "http://localhost:800/view/Buildampel/"
    #startDate = datetime.combine(date(2020, 3, 14), datetime.min.time())
    startDate = date(2020, 3, 10)
    endDate = date(2020, 3, 15)
    #view = jenkins_connector.connect_to_jenkins(url)
    jenkins = jenkins_service.connect_to_jenkins(url)
    jobname = "fahrzeug-server"
    job = jenkins_service.get_job_by_name(jenkins, jobname)
    build_dto_list = jenkins_service.get_builds_from_build_ids(job, job.get_build_ids())
    #relevant_builds = build_service.filter_builds_by_time_range(build_dto_list, startDate, endDate)
    #initial_status = "SUCCESS"
    fail_time_list = build_service.get_list_of_fail_times_from_builds(build_dto_list)
    export_data = exporter_service.create_export_format(jobname, fail_time_list)
    exporter_service.export_to_csv(export_data, "jenkinsStatusStatistic.csv")

    print("failTimeList: ")
    print(failTimeList)
    assert len(failTimeList) == 10